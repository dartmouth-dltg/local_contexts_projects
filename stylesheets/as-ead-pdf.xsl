...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="toc">
    <fo:block font-size="11pt" margin-top="12pt" margin-bottom="18pt">
      <fo:basic-link text-decoration="none" internal-destination="toc" color="#0D6CB6"><fo:inline
          font-weight="bold">^</fo:inline> Return to Table of Contents </fo:basic-link>
    </fo:block>
  </xsl:template>
    ...
    <fo:block-container max-width="6.5in" max-height="4in">
      <fo:block text-align="left">
        <fo:external-graphic content-width="scale-down-to-fit" content-height="scale-down-to-fit"
          width="200px" scaling="uniform">
          <xsl:attribute name="src">
            <xsl:value-of select="$pdf_image"/>
          </xsl:attribute>
        </fo:external-graphic>
      </fo:block>
    </fo:block-container>
              ...
              <xsl:if test="self::ead:origination">
                <xsl:choose>
                  ...
                  <xsl:when test="ead:persname[@role != '']"> - <xsl:value-of
                      select="ead:persname/@role_translation"/>
                  </xsl:when>
                  <xsl:otherwise/>
                </xsl:choose>
              </xsl:if>
  ...
  <xsl:template match="ead:langmaterial" mode="overview">
    <fo:table-row>
      <fo:table-cell padding-bottom="8pt" padding-right="16pt" text-align="right" font-weight="bold">
        <fo:block>
          <xsl:value-of select="local:tagName(.)"/>: </fo:block>
      </fo:table-cell>
      <fo:table-cell>
        <fo:block>
          <xsl:choose>
              ...
              <xsl:attribute name="number-rows-spanned">
                <xsl:choose>
                  <xsl:when test="ead:did/ead:container/@label">
                    <xsl:value-of select="count(ead:did/ead:container[@label]) + 1"/>
                  </xsl:when>
      ...
      <xsl:when test="ead:did/ead:container[@label]">
        <fo:table-row border-top="1px solid #ccc" keep-with-next.within-page="always">
          <fo:table-cell margin-left="{$clevelMargin}" padding-top="4pt"
            ...
            <xsl:apply-templates mode="dsc" select="
  ...
  <xsl:template match="ead:did" mode="dscSeriesTitle">
    <fo:block font-weight="bold" font-size="12" margin-bottom="0" margin-top="4"
    ...
    <xsl:if test="normalize-space()">
      <fo:block xsl:use-attribute-sets="smpDsc">
        ...
        <xsl:if test="@certainty">
          <fo:inline font-style="italic"> (<xsl:value-of select="@certainty"/>)</fo:inline></xsl:if>
      </fo:block>
    </xsl:if>
  </xsl:template>
  <xsl:template match="
      ead:relatedmaterial | ead:separatedmaterial | ead:accessrestrict | ead:userestrict |
      ead:custodhist | ead:accruals | ead:altformavail | ead:processinfo | ead:appraisal | ead:originalsloc"
    mode="dsc">
    <xsl:if test="child::*">
      <fo:block xsl:use-attribute-sets="smpDsc">
